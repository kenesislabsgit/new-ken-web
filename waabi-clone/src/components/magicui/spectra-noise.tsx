...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  uv += normalize(diff + 0.001) * influence * uMouseStrength * -0.1;
  // Swirl near cursor
  float swirl = influence * uMouseStrength * 0.5;
  float cs = cos(swirl), sn = sin(swirl);
  vec2 centered = uv - mouseUV;
  uv = mouseUV + vec2(centered.x*cs - centered.y*sn, centered.x*sn + centered.y*cs);
